...
//
//        try {
//            ResultSet lec = connectDB.createStatement().executeQuery("SELECT * FROM tutorial");
//            while(lec.next()) {
//                if(!lec.getString("tutorial_id").equals("NONE")){
//                    lectid.add(lec.getString("tutorial_id"));
//                    lectday.add(lec.getString("tutorial_day"));
//                    lectstart.add(lec.getString("tutorial_start_time"));
//                    lectend.add(lec.getString("tutorial_end_time"));
//                }
//            }
//
//
...
//
//            try {
//                PreparedStatement statement = connectDB.prepareStatement("INSERT INTO staff_teach_tutorial VALUES (?,?)");
...
//    }
    ...
    public void all(){
        try {
            ResultSet lec = connectDB.createStatement().executeQuery("SELECT staff_teach_lecture.staff_id, occ.occ_id FROM occ\n" +
                                                            "INNER JOIN staff_teach_lecture ON staff_teach_lecture.lecture_id=occ.lecture_id\n" +
                                                            "UNION \n" +
                                                            "SELECT staff_teach_tutorial.staff_id, occ.occ_id FROM occ\n" +
                                                            "INNER JOIN staff_teach_tutorial ON staff_teach_tutorial.tutorial_id=occ.tutorial_id\n" +
                                                            "UNION \n" +
                                                            "SELECT staff_teach_lab.staff_id, occ.occ_id FROM occ\n" +
                                                            "INNER JOIN staff_teach_lab ON staff_teach_lab.lab_id=occ.lab_id");
            while(lec.next()) {
                staffid.add(lec.getString("staff_id"));
                courseid.add(lec.getString("occ_id"));
            }
        ...
        for (int i = 0; i < staffid.size(); i++) {
            try {
                PreparedStatement statement = connectDB.prepareStatement("INSERT INTO staff_teach_course VALUES (?,?)");
                statement.setString(1,staffid.get(i));
                statement.setString(2,courseid.get(i).substring(0, 7));
                statement.executeUpdate();
            } catch(Exception e) {
                e.printStackTrace();
                e.getCause();
